...
//cGraphics drawing overloads 
		...
		{  
			cVector3 normal = ( corner1 - corner0 ).mult( corner2 -  corner0 ); 
		...
		{ // DON'T COPY 
			...
			{ 
				circlevert[ i ] = center + new cVector3( radius * (float) Math.Cos( angle ), radius * (float) Math.Sin( angle ), 0.0f ); 
				angle += angleinc; 
			} 
		...
		public override void drawShape( cShape ppolygon, int drawflags, bool topPoint = false ) 
		{ 
		...
		//Find lighting, and decide whether to draw fill, draw edges, or both.  
            bool drawfill = ppolygon.Filled && (( drawflags & ACView.DF_WIREFRAME) == 0); 
				 ...
				 wireframe mode. */
            bool drawedges = ( ppolygon.Edged || ( (drawflags & ACView.DF_WIREFRAME) != 0 )); 
	...
	"sees" is the actual values used in the vertex arrays. */
        ...
        The stride = 0 means don't skip over any vertices. */
            ...
            { 
                ...
                for ( i = 0; i < size; i++ )
                {
                    sumX += ppolygon.getVertex(i).X;
                    sumY += ppolygon.getVertex(i).Y;
                }
                topX = sumX / size;
                topY = sumY / size;
                for (i = 0; i < size; i++)
			...
			if ( ppolygon.vertCount() >= 3 ) 
				upnormal = ppolygon.getVertex( 1 ) - ppolygon.getVertex( 0 ).mult( 
	...
	in the plane z = prismdz */ 
			if ( prismdz != 0.0f ) 
			{
                ...
                if (topPoint)
                {
                    for (i = 0; i < size; i++)
                    {
                        vertices[3 * (size) + 3 * i] = ppolygon.getVertex(i).X;
                        vertices[3 * (size) + 3 * i + 1] = ppolygon.getVertex(i).Y;
	...
	OpenGL lets us store such a "rat's nest" index order in an array. */ 
				...
				for ( i = 0; i < size -1; i++) 
				{ 
					edgenormal = ( ppolygon.getVertex( i + 1 ) - ppolygon.getVertex( i )).mult( 
						upnormal ); 
			...
			//Have to do the last edge normal as a special case as you wrap around to 0 
				i = size -1; 
				edgenormal = ppolygon.getVertex( 0 ) - ppolygon.getVertex( i )* 0.5f; 
				edgenormal = edgenormal.mult( upnormal ); 
				edgenormals[ 3 * i ] = edgenormal.X; 
				edgenormals[ 3 * i + 1 ] = edgenormal.Y; 
				edgenormals[ 3 * i + 2 ] = edgenormal.Z; 
		...
		//Draw the edge line of the bottom polygon 
				GL.LineWidth( ppolygon.LineWidth ); 
		...
		public override void drawstarShape( cShape ppolygon, int drawflags ) 
		{ 
			int i; 
