...
//            _metrickey = -1; //Put in a bad index by default 
            ...
            bool treatascontinuousmotion = false)
        {
            if (amount < 0.0f)
                ...
                amount = 1.0f;
            return moveTo((1.0f - amount) * _position + amount * newposition, treatascontinuousmotion);
        }
        ...
        on the _movebox.zsize(). */
            if (dt > 0.00001f) //Don't divide by 0.
                Velocity = (newposition - _position) * (1.0f / dt);
        ...
        public void addVelocity(cVector3 velocitychange) { Velocity = _velocity + velocitychange; }
        ...
        {
            _tangent.normalize();
            _normal -= _tangent * ( _normal * _tangent); //Make it perpendicular to _tangent.
        ...
        public void addAcceleration(cVector3 acceleration) { _acceleration += acceleration; }
        ...
        public void rotateAttitude(cSpin spin)
        {
            ...
            _attitude *= cMatrix3.rotation(spin);
        }
        ...
        {
            cVector3 newtangent = targetpos - Position;
        ...
        public bool lookAtProportional(cVector3 targetpos, float amount)
        {
            if (amount < 0.0f)
                ...
                amount = 1.0f;
            cVector3 targettangent = (targetpos - _position).normalize();
            cVector3 weightedlookpos = _position +
                AttitudeTangent * (1.0f - amount) + targettangent * amount;
        ...
        public void randomizeVelocity(float minspeed, float maxspeed, bool force2D = false)
        {
            ...
            Velocity = Framework.randomOb.randomReal(minspeed, maxspeed) * randomunitvector;
        }
        ...
        { //Note that cCritterWall has its own version of this method.
            if ((mutationflags & MF_NUDGE) != 0) //Special kind of weak mutation 
            {
                ...
                _velocity *= Framework.randomOb.randomReal(0.5f, 1.5f);
        ...
        { // This helper method is only called by cCritter.move.
            ...
            {
                _outcode = effectiveborder.addBounce(ref _position, ref _velocity, _bounciness, dt); /* The _position and
            ...
            else if (_wrapflag == cCritter.WRAP)
            {
                ...
                BUGFIX this broke the wall collide code!!!! */
                    _oldposition = _position - _velocity * dt; /* This way the old position is
        ...
        { //This sets _speed and _tangent to match _velocity 
            ...
            if (_speed > 0.00001f) /*  Don't allow small positive speeds as they cause wobble,
                ...
                _tangent = _velocity * (1.0f / _speed);
            else
                _speed = 0.0f;
        }
        ...
        { // This helper method is called by cCritter.move, by cCritter.setVelocity and cCritter.setTangent.
            ...
            cVector3 tangentchange = _tangent - _oldtangent;
        ...
        latest turn, that is, guess that newnormral is (_tangent-olddirection).normalize(). */
            tangentchange = tangentchange - _tangent * ( tangentchange * _tangent); //Make it perpendicular to _tangent.
            _curvature = tangentchange.normalizeAndReturnMagnitude();
            /*Make it a unit vector.  Also save the size of it as your curvature. */
            float howparallel = tangentchange * _normal;
        ...
        method to find a new value for _normal. */
            {
                ...
                _normal.normalize();
                _normal -= _oldtangent * ( _normal * _oldtangent ); //Make it perpendicular to _oldtangent.
                _normal.normalize(); //Make it a unit vector.
        ...
        public virtual bool collide(cCritter pother )
        {
            ...
            give = toOther * givecomponent;
            receivecomponent = (pother._velocity) * toOther;
            receive = toOther * receivecomponent; //else leave it at 0.
        ...
        of collision and add in the new components. */
            if (!_absorberflag && !_fixedflag)
                Velocity = _velocity - give + 
                    give *_bounciness * massdivisor * (1.0f - massratio) + receive * 2.0f * massratio;
            if (!pother.AbsorberFlag && !pother.FixedFlag)
                pother.Velocity = pother._velocity - receive +
                    (give * 2.0f + receive * (massratio - 1.0f)) * pother.Bounciness * massdivisor;
        ...
        which is the multiplier of toOther in the line just below. */
            if (!_fixedflag && !pother.FixedFlag)
            {
                contactpoint = _position + 
                    toOther * ((Radius * distanceToOther) / (Radius + pother.Radius));
                moveTo(contactpoint - toOther * Radius, true); /* The TRUE arg here means that
				we should treat motion as a physical thing that can change the
				_tangent, _normal, and _binormal. */
                pother.moveTo(contactpoint + toOther * pother.Radius, true);
            }
        ...
        as a bumper in a pinball game. */
                ...
                pother.moveTo(_position + toOther * (Radius + pother.Radius), true);
            else if (!_fixedflag && pother.FixedFlag)
                moveTo(pother.Position - toOther * (Radius + pother.Radius), true);
        ...
        /// <param name="pother">The critter used to check for a collision with this critter.</param>
        /// <returns>Returns true if a collision took place; otherwise returns false.</returns>
        public virtual bool collide(cCritter pother, float extraDistance )
        {
            cVector3 toOther = new cVector3();
            float distanceToOther;
            float givecomponent;
            ...
            cDistanceAndDirection distanceanddirection = distanceAndDirectionTo(pother);
            distanceToOther = distanceanddirection._distance;
            if (distanceToOther > Radius + pother.Radius + extraDistance )
                return false;
            toOther.copy(distanceanddirection._direction);
            givecomponent = _velocity * toOther;
            give = toOther * givecomponent;
        ...
        public virtual void feelforce()
        {
            ...
            foreach (cForce f in _forcelist)
                forcesum += _forcelist.ElementAt().force(this);
        ...
        public virtual void drawHighlight(cGraphics pgraphics, float highlightsizeratio)
        {
            ...
            pgraphics.translate(Sprite.Center + (new cVector3(0.0f, 0.0f, 1.0f)) * 0.01f);
        ...
        public int move(float dt)
        {
            ...
            if (_speed > _maxspeed)
                _speed = _maxspeed;
            _velocity = _tangent * _speed; //Reset the _velocity in case you changed _speed.
...
//        }
        ...
        public virtual cSprite Sprite
        {
            ...
            {
                _psprite = value;
//                _psprite.PrismDz = _defaultprismdz; //Do I need this?  // Childs answer -- no, and it caused me a big problem 
