...
namespace ACFramework
{
    class cGame
    {
        ...
        public virtual void removeReferencesTo(cCritter pdeadcritter)
        {
            ...
            //works        if (!( pdeadcritter is cCritterWall))
    ...
    an immediate add, you set immediateadd to TRUE. */
        ...
        */
        ...
        public cSprite randomSprite(int spritetypeindex)
        {
            ...
            {
                newpoly = new cShape(Framework.randomOb.random(3, 5));
            ...
            else if (spritetypeindex == cGame.ST_FANCYPOLYGONS)
            {
                newpoly = new cShape();
                newpoly.randomize( //cSprite.MF_RADIUS |  
                    cShape.MF_COLOR | cShape.MF_LINEWIDTH | cShape.MF_DOTS | cShape.MF_VERTCOUNT);
                return newpoly;
            }
            else if (spritetypeindex == cGame.ST_ASTEROIDPOLYGONS)
            {
                newpoly = new cShape();
                newpoly.setRandomAsteroidShape(5, 20, Framework.randomOb.randomReal(0.0f, 0.4f));
                newpoly.randomize( //cSprite.MF_RADIUS  
                    cShape.MF_COLOR);
                return newpoly;
            }
            else if (spritetypeindex == cGame.ST_BUBBLES)
            {
                ...
                newbubble.randomize( //cSprite.MF_RADIUS |  
                    cShape.MF_COLOR | cShape.MF_LINEWIDTH);
                return newbubble;
            }
            else if (spritetypeindex == cGame.ST_SPHERES)
            {
                psphere = new cSphere( 1.0f, Color.Gray );
                psphere.randomize(cShape.MF_COLOR);
            ...
            else if (spritetypeindex == cGame.ST_POLYPOLYGONS)
            {
                ...
                newpolypoly.randomize( //cSprite.MF_RADIUS |  
                    cShape.MF_COLOR | cShape.MF_LINEWIDTH | cShape.MF_DOTS |
                    cShape.MF_VERTCOUNT);
                return newpolypoly;
            }
            ...
            {
                newpolypoly = new cPolyPolygon();
                newpolypoly.randomize(cShape.MF_VERTCOUNT);
