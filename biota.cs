...
//            }
    ...
    Instead of cloning the critter I will copy the critter params and let this call clone the sprite. */ 
		        ...
		        ElementAt( ).mutate( cSprite.MF_ALL | cShape.MF_ALL ); 
		...
		private void _zap( ) 
		{ 
			ElementAt( ).randomize( cCritter.MF_VELOCITY | cSprite.MF_ALL | cShape.MF_ALL ); 
		} 
    ...
    //LinkedList Overloads 
		...
		the other. */ 
			...
			{ 
				...
				case. */ 
				if (!( ElementAt( 0 ) is cCritterArmedPlayer)) 
		...
		{ //See comment on purgeCritters.  purgeNonPlayerCritters gets rid of all but player.
            foreach ( cCritter c in this )
                if ( ElementAt() != _pgame.Player && !( ElementAt() is cCritterWall ))
                    ElementAt().delete_me();
			processServiceRequests(); 	
		} 
        ...
        public void purgeCritters<DataType>( ) 
	...
	//Accessor 
        ...
        public int count<DataType>( bool includesubclasses ) 
		{ 
			int found = 0; 
            foreach ( cCritter c in this )
                if ( (includesubclasses && ElementAt() is DataType) ||
                    ( !includesubclasses && ElementAt().ToString() == typeof(DataType).ToString() ))
                        found++;
			return found; 
		} 
	...
	//Distance and touch  methods 
		...
		public cCritter pickClosestCritter<DataType>( cCritter pcrittercenter )
		{ 
			cCritter pclose = null; 
			cCritter ptest; 
			float closestdistance = 1000000000.0f ; 
        ...
        public cCritter pickClosestCritter<DataType>( cCritter pcrittercenter, bool includesubclasses ) 
		{ 
			cCritter pclose = null; 
			cCritter ptest; 
			float closestdistance = 1000000000.0f ; 
            ...
            foreach ( cCritter c in this )
            {
                ...
                if ( ptest == pcrittercenter )
                    continue;
				if (!( 
					( includesubclasses && ptest is DataType ) || 
					( !includesubclasses && ptest.RuntimeClass == typeof(DataType).ToString() ) 
				)) 
                    continue;
				testdistance = pcrittercenter.distanceTo( ptest ); 
				if ( testdistance < closestdistance ) 
        ...
        public cCritter pickClosestAhead<DataType>( cLine sightline, cCritter pcrittercenter, float visionangle ) 
		{ 
			float closestdistance = 1000000000.0f , testdistance, angle; 
		    cCritter closest = null;    	
        ...
        public cCritter pickClosestAhead<DataType>( cLine sightline, cCritter pcrittercenter, 
            float visionangle, bool includesubclasses ) 
		{ 
			float closestdistance = 1000000000.0f , testdistance, angle; 
		    cCritter closest = null;    	
            ...
            foreach ( cCritter c in this )
            {
				...
				if (!( //Only go for the right type of critter.
					( includesubclasses && ppossible is DataType) || 
					( !includesubclasses && ppossible.RuntimeClass == typeof(DataType).ToString() ) 
				)) 
					continue; //Skip this critter.
				if ( ppossible == pcrittercenter || Math.Abs( angle ) > visionangle / 2.0f ) //Only go for those ahead.
					continue; //Skip this critter.
