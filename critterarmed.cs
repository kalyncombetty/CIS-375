...
{ 
	...
	//Nonserialized pointer ref.
		...
		{ 
			...
			float bulletdistance = ( bulletdistance1 > bulletdistance2 )? bulletdistance1
                : bulletdistance2; 
			cVector3 end = start + _pshooter.AimVector * bulletdistance; 
	...
	//Special methods 
        ...
        /// modify this function using is for the cCritterWall.  Note also that damage will not be done if
	...
	{ 
		...
		{ 
            ...
            cSpriteSphere bulletsprite = new cSphere(cCritter.BULLETRADIUS, Color.Red );
	...
	{ 
		...
		{ 
			...
			pgame()->buildCollider(); */
            cSpriteSphere bulletsprite = new cSphere(cCritter.BULLETRADIUS, Color.LightGreen );
			Sprite = bulletsprite; /* Also sets cSprite._prismdz to CritterBullet.BULLETPRISMDZ. */ 
		}
	...
	{ 
		...
		{ 
            ...
            cSpriteSphere bulletsprite = new cSphere(cCritter.BULLETRADIUS, Color.LightBlue );
			Sprite = bulletsprite; /* Also sets cSprite._prismdz to CritterBullet.BULLETPRISMDZ. */ 
		}
	...
	//Mutators 
		...
		{ 
			if ( !_armed ) 
				return ; 
			AimVector = ( vclick - _position ).Direction; /* Use the setAimVector call as this
	...
	drawflag. */ 
				...
				cVector3 end = start + _aimvector * _gunlength * Radius; 
			} 
		...
		public virtual cCritterBullet BulletClass
		{
			...
			set
            {
                if (value is cCritterBulletSilver && this is cCritterArmedPlayer)
                    MessageBox.Show("ERROR:  The player cannot use cCritterBulletSilver or one of its derived classes");
                if (this is cCritterArmedRobot && !(value is cCritterBulletSilver
                    || value is cCritterBulletRubber))
	...
	being copied is a cCritterArmed, and if it is then it copies the additional fields. */ 
			base.copy( pcritter ); 	
			if ( !( pcritter is cCritterArmedPlayer )) 
				return ; //You're done if pcritter isn't a cCritterArmed*.
	...
	//Main methods 
        ...
        public override bool collide( cCritter pcritter ) 
		{
			...
			if ( collided && _sensitive && !( pcritter is cCritterWall )) 
				damage( 1 ); 
			return collided; 
		} 
		...
		pbullet->addVelocity(_velocity), but this gives unattractive results. */ 
			float bulletspeedup = _velocity * pbullet.Tangent; 
			if ( bulletspeedup > 0.0f ) 
				pbullet.addVelocity( pbullet.Tangent * bulletspeedup ); //So bullets don't stack up.
			return pbullet; 
		}
