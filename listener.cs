...
namespace ACFramework
{
    ...
    class cListenerArrowAttitude : cListener
    {
        ...
        public override void listen(float dt, cCritter pcritter) /* used in Defender3d */
        {
            ...
            the arrow key direction, and then match the attitude. */
            if (left)
                pcritter.Velocity = pcritter.AttitudeNormal * pcritter.MaxSpeed;
            if (right)
                pcritter.Velocity = pcritter.AttitudeNormal * -pcritter.MaxSpeed;
            if (pagedown)
                pcritter.Velocity = pcritter.AttitudeTangent * -pcritter.MaxSpeed;
            if (pageup)
                pcritter.Velocity = pcritter.AttitudeTangent * pcritter.MaxSpeed;
            if (down)
                pcritter.Velocity = pcritter.AttitudeBinormal * -pcritter.MaxSpeed;
            if (up)
                pcritter.Velocity = pcritter.AttitudeBinormal * pcritter.MaxSpeed;
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (up)
                pcritter.addAcceleration(
                    pcritter.AttitudeTangent * pcritter.ListenerAcceleration);
            if (down)
                pcritter.addAcceleration(
                    pcritter.AttitudeTangent * -pcritter.ListenerAcceleration);
            bool inreverse = pcritter.Tangent * pcritter.AttitudeTangent < 0.0f;
    ...
    class cListenerCursor : cListener
    {
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (_attached && dt > 0.00001f) //Check dt so don't divide by zero. (_attached always TRUE) 
            {
                ...
                cVector3 displacement = cursorposclamp - pcritter.Position;
                ...
                //The if condition is because you get wobble if you adjust for too small a step.
                {
                    ...
                    not acceleration. */
                    pcritter.Velocity = displacement * (1.0f / dt);
                    pcritter.copyMotionMatrixToAttitudeMatrix();
                }
            }
    ...
    class cListenerViewerRide : cListener //Assume 3D 
    {
        ...
        public override void install(cCritter pcritter)
        {
            ...
            _offset = cListenerViewerRide.OFFSETDIR * pcritter.Player.Radius;
        }
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (up && control)
                _offset += new cVector3(0.0f, 0.0f, RIDESTEP);
    ...
    class cListenerViewerFly : cListener //Assume 3D 
    {
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (up && control && !shift)
                pcritter.moveTo(pcritter.Position - pcritter.Binormal * MOVEVIEW * dt * pcritter.MaxSpeed);
            if (down && control && !shift)
                pcritter.moveTo(pcritter.Position + pcritter.Binormal * MOVEVIEW * dt * pcritter.MaxSpeed);
            if (left && control && !shift)
                pcritter.moveTo(pcritter.Position - pcritter.Normal * MOVEVIEW * dt * pcritter.MaxSpeed);
            if (right && control && !shift)
                pcritter.moveTo(pcritter.Position + pcritter.Normal * MOVEVIEW * dt * pcritter.MaxSpeed);
            if (pageup && control && !shift)
                pcritter.moveTo(pcritter.Position + pcritter.Tangent * dt * pcritter.MaxSpeed);
    ...
    class cListenerScooterLevitator : cListener
    {
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (up)
                pcritter.Velocity = pcritter.AttitudeTangent * 0.1f * pcritter.MaxSpeed;
            if (down)
                pcritter.Velocity = pcritter.AttitudeTangent * 0.1f * -pcritter.MaxSpeed;
            if (flyup)
