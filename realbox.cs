...
namespace ACFramework
{
	...
	cRealBox2. */
        ...
        public int addBounce(ref cVector2 position, cVector2 velocity, float bounciness)
        {
        ...
        public int addBounce(ref cVector2 position, ref cVector2 velocity, float bounciness, float dt)
        {
            if (Math.Abs(dt) < 0.00001f)
                return BOX_INSIDE; //You're not moving.
            cVector2 dtvelocity = velocity * dt;
            int outcode = addBounce(ref position, dtvelocity, bounciness);
	...
	cRealBox3. */
        ...
        public int addBounce(ref cVector3 position, cVector3 velocity, float bounciness)
        {
            int outcode = BOX_INSIDE;
        ...
        public int addBounce(ref cVector3 position, ref cVector3 velocity, float bounciness, float dt)
        {
            if (Math.Abs(dt) < 0.00001f)
                return BOX_INSIDE; //You're not moving.
            cVector3 dtvelocity = velocity * dt;
            int outcode = addBounce(ref position, dtvelocity, bounciness);
            velocity = dtvelocity / dt;
            return outcode;
        }
